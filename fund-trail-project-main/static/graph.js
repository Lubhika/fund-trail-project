// Branch data from backend (pre-resolved via IFSC_CODES.xlsx)
const branchCache = new Map();
const isViewer = typeof window !== "undefined" ? Boolean(window.isViewerRole) : false;

async function fetchBranchInfo(ifsc) {
  if (!ifsc) return { BRANCH: 'Unknown', PHONE: '' };
  if (branchCache.has(ifsc)) {
    return { BRANCH: branchCache.get(ifsc), PHONE: '' };
  }
  try {
    const res = await fetch(`/ifsc_info/${encodeURIComponent(ifsc)}`);
    if (!res.ok) throw new Error('Failed to fetch IFSC info');
    const data = await res.json();
    const branch = data?.BRANCH || 'Unknown';
    const phone = data?.PHONE || '';
    branchCache.set(ifsc, branch);
    return { BRANCH: branch, PHONE: phone };
  } catch {
    branchCache.set(ifsc, 'Unknown');
    return { BRANCH: 'Unknown', PHONE: '' };
  }
}
...
async function populateBranchNames(root) {
  ...
  nodesWithIfsc.forEach(n => {
    if (n.data.branch) {
      branchCache.set(n.data.ifsc, n.data.branch);
    }
  });
}
      ...
      if (d.data.ifsc) {
        ...
        if (!isRepeated) {
          baseHtml +=
            ...
            `<div class="detail-row"><span class="label">Txn ID:</span> ${d.data.txn_id || d.data.txid || '—'}</div>` +
        ...
        baseHtml += `<div class="detail-row"><span class="label">Bank Phone:</span> ${d.data.phone || '—'}</div>`;
        detailsContent.innerHTML = baseHtml;
        if ((!d.data.phone || d.data.phone === '') && d.data.ifsc) {
          fetchBranchInfo(d.data.ifsc).then(info => {
            d.data.phone = info.PHONE || '';
            const rows = detailsContent.querySelectorAll('.detail-row');
            rows.forEach(row => {
              const labelEl = row.querySelector('.label');
              if (labelEl && labelEl.textContent.trim() === 'Bank Phone:') {
                row.innerHTML = `<span class="label">Bank Phone:</span> ${d.data.phone || '—'}`;
              }
            });
          }).catch(() => {});
        }
